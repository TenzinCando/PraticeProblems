...
public class DivideTwoIntegers {
    ...
    public int divide(int dividend, int divisor) {
        ...
        if (dividend == 0 ||
            ...
            (divisor > 0 && dividend < 0 && Math.abs(dividend) < divisor)) return 0;
    ...
    */
    private String partialDivider(String dividend, int divisor) {
        ...
        for(char digit: dividend.toCharArray()) {          
            ...
            String divisible = "";
            int partialDividendNumber = Integer.parseInt(singleDividendDigit.toString());
            int sum = divisor;
            ...
            if (divisor > partialDividendNumber) {
                ...
                // get the number of times divisor goes into partialdividendNumber
                // TODO: Create a map to store already visited numbers
                int counter = 1;
                while (sum <= partialDividendNumber) {
                    //System.out.println("Dividend: " + dividend + "| Sum: " + sum + "| Counter: " + counter);
                    counter++;
                    try {
            ...
            if (divisible != "0" ) {
                //System.out.println("PartialDividend: " + partialDividend.toString() + "| Sum: " + sum);                
                int newPartialDividend = Integer.parseInt(singleDividendDigit.toString()) - sum;
                singleDividendDigit.setLength(0);
                singleDividendDigit.append(newPartialDividend);
            }
            
            divisibleNumber += divisible;
            //System.out.println("Digit: " + digit + "| divisor: " + divisor + "| divisible: " + divisible + "| PartialDividend: " + partialDividend.toString() + "| Counter: " + divisibleNumber);
        }
